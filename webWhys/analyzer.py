...
"""
        ...
        try:
            logger.debug(f"LLM: inferring audience with {self.model}")
            ...
            logger.debug(f"LLM audience inference succeeded: {response.choices[0].message.content.strip()}")

            inferred = response.choices[0].message.content.strip()
            logger.debug(f"LLM audience raw response: '{inferred}'")
    ...
    async def _generate_llm_recommendations(
        ...
        try:
            logger.debug(f"LLM: generating recommendations with {self.model}")
...
8. REFLECT THE BRAND'S ACTUAL PRIMARY MESSAGE: Read the brand documents carefully. Use the positioning angles, audience language, and differentiators that are most prominent in those docs — not generic industry buzzwords. If the brand documents don't emphasize a particular term or concept, don't lead with it. Mirror the hierarchy and priorities in the docs."""
            ...
            logger.debug(f"LLM recommendations succeeded: {len(result.get('recommendations', []))} items")
            return {
            ...
            error_msg = str(e)
            logger.error(f"LLM recommendations failed: {error_msg}")
            import traceback
            traceback.print_exc()
